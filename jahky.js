...
readdirSync('./src/commands/Global', { encoding: 'utf8' }).filter(file => file.endsWith(".js")).forEach((files) => {
    let command = require(`./src/commands/Global/${files}`);
...
readdirSync('./src/commands/Register', { encoding: 'utf8' }).filter(file => file.endsWith(".js")).forEach((files) => {
    let command = require(`./src/commands/Register/${files}`);
    console.log(`[Register command] ${command.name} loaded!`)
    commands.set(command.name, command);
    if (!command.aliases || command.aliases.length < 1) return
    command.aliases.forEach((otherUses) => { aliases.set(otherUses, command.name); })
})

readdirSync('./src/commands/Owner', { encoding: 'utf8' }).filter(file => file.endsWith(".js")).forEach((files) => {
    let command = require(`./src/commands/Owner/${files}`);
    console.log(`[Owner command] ${command.name} loaded!`)
    commands.set(command.name, command);
    if (!command.aliases || command.aliases.length < 1) return
    command.aliases.forEach((otherUses) => { aliases.set(otherUses, command.name); })
})

readdirSync('./src/commands/Penal', { encoding: 'utf8' }).filter(file => file.endsWith(".js")).forEach((files) => {
    let command = require(`./src/commands/Penal/${files}`);
    console.log(`[Penal command] ${command.name} loaded!`)
    commands.set(command.name, command);
    if (!command.aliases || command.aliases.length < 1) return
    command.aliases.forEach((otherUses) => { aliases.set(otherUses, command.name); })
})

readdirSync('./src/commands/Talent', { encoding: 'utf8' }).filter(file => file.endsWith(".js")).forEach((files) => {
    let command = require(`./src/commands/Talent/${files}`);
    console.log(`[Talent command] ${command.name} loaded!`)
    commands.set(command.name, command);
    if (!command.aliases || command.aliases.length < 1) return
    command.aliases.forEach((otherUses) => { aliases.set(otherUses, command.name); })
})

readdirSync('./src/commands/Advanced', { encoding: 'utf8' }).filter(file => file.endsWith(".js")).forEach((files) => {
    let command = require(`./src/commands/Advanced/${files}`);
    console.log(`[Advanced command] ${command.name} loaded!`)
    commands.set(command.name, command);
    if (!command.aliases || command.aliases.length < 1) return
    command.aliases.forEach((otherUses) => { aliases.set(otherUses, command.name); })
})

readdirSync('./src/commands/İnfo', { encoding: 'utf8' }).filter(file => file.endsWith(".js")).forEach((files) => {
    let command = require(`./src/commands/İnfo/${files}`);
    console.log(`[İnfo command] ${command.name} loaded!`)
    commands.set(command.name, command);
    if (!command.aliases || command.aliases.length < 1) return
    command.aliases.forEach((otherUses) => { aliases.set(otherUses, command.name); })
})
...
client.on('message', message => {
    ...
    if ([".tag", "!tag", "tag", "TAG"].some(x => message.content.toLowerCase.startsWith(x))) return message.channel.send(`\`${config.registeration.GuilDTag}\``)
...
client.on("voiceStateUpdate", async (oldState, newState) => {
    const channel = newState.guild.channels.cache.get(config.logs.voicelog);
    if (!channel) return;
...
client.on("rateLimit", RateLimitData => console.log("RATE LIMIT WARN",RateLimitData))
